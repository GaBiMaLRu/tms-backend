...
import com.trans.tms.common.utils.IdCardUtils;
import com.trans.tms.common.utils.PageResponse;
import com.trans.tms.common.utils.Result;
import com.trans.tms.entity.Member;
import com.trans.tms.enums.MemberIdCardVerifyStatus;
import com.trans.tms.enums.OrderStatus;
import com.trans.tms.enums.OrderType;
import com.trans.tms.enums.pickuptask.PickupDispatchTaskSignStatus;
...
import com.trans.tms.enums.transportorder.TransportOrderSchedulingStatus;
...
import com.trans.tms.feign.courier.AppCourierFeign;
...
public class CourierController {
    ...
    public Result pickupDispatch(Integer page, Integer pagesize, Integer taskType, Integer status, String keyword, String date) {
        ...
        if (result.getItems() == null || result.getItems().size() == 0) {
            return Result.ok().put("data", PageResponse.<PickupDispatchDTO>builder().current(result.getCurrent()).size(result.getSize()).total(result.getTotal()).counts(result.getCounts()).build());
        }
        ...
        return Result.ok().put("data", PageResponse.<PickupDispatchDTO>builder()
                ...
                .size(result.getSize())
                .total(result.getTotal()).counts(result.getCounts())
