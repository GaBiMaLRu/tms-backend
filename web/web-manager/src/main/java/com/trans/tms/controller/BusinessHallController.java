...
import com.trans.tms.common.utils.EntCoordSyncJob;
import com.trans.tms.common.utils.PageResponse;
import com.trans.tms.common.utils.Result;
import com.trans.tms.feign.agency.AgencyScopeFeign;
...
import com.trans.tms.vo.base.AreaSimpleVo;
import com.trans.tms.vo.base.businessHall.CourierScopeVo;
import com.trans.tms.vo.base.businessHall.GoodsTypeVo;
import com.trans.tms.vo.base.transforCenter.business.TruckTypeVo;
...
public class BusinessHallController extends BaseController {
    ...
    public PageResponse<GoodsTypeVo> findGoodsTypeByPage(@RequestParam(name = "name", required = false) String name,
        ...
        PageResponse<GoodsTypeDto> goodsTypePage = goodsTypeFeign.findByPage(getPageNo(), getPageSize(), name, truckTypeId, truckTypeName);
        //加工数据
        ...
        return PageResponse.<GoodsTypeVo>builder().items(goodsTypeVoList).size(getPageNo()).current(getPageSize()).counts(goodsTypePage.getCounts()).total(goodsTypePage.getTotal()).build();
    }
    ...
    public PageResponse<SysUserVo> findCourierByPage(@RequestParam(name = "page") Integer page,
        ...
        if (result.getIsSuccess() && result.getData() != null) {
            ...
            return PageResponse.<SysUserVo>builder().items(voList).current(page).size(pageSize).counts(userPage.getTotal()).total(userPage.getPages()).build();
        }
        return PageResponse.<SysUserVo>builder().items(new ArrayList<>()).current(page).size(pageSize).counts(0L).total(0L).build();
    }
