...
import com.trans.tms.DTO.*;
import com.trans.tms.dto.angency.AgencyScopeDto;
...
import com.trans.tms.common.enums.ErrorCode;
...
import com.trans.tms.enums.OrderStatus;
import com.trans.tms.enums.driverjob.DriverJobStatus;
import com.trans.tms.enums.pickuptask.PickupDispatchTaskAssignedStatus;
import com.trans.tms.enums.pickuptask.PickupDispatchTaskStatus;
import com.trans.tms.enums.pickuptask.PickupDispatchTaskType;
import com.trans.tms.enums.transportorder.TransportOrderStatus;
import com.trans.tms.enums.transporttask.TransportTaskStatus;
import com.trans.tms.feign.*;
...
import com.trans.tms.vo.AgencyVo;
...
// 查询地址
            ...
            if (CollectionUtils.isEmpty(cargoTranTaskDTOS)) {
                return Result.ok().put("data", PageResponse.<CargoTranTaskDTO>builder()
                        .counts(0L).current(page).size(pagesize).total(0L)
                        .build());
            }
            ...
            return Result.ok().put("data", PageResponse.<CargoTranTaskDTO>builder()
                    .counts(result.getCounts()).current(page).size(pagesize).total(result.getTotal())
    ...
    public Result orders(String keyword, String id) {
        ...
        if (StringUtils.isBlank(id)) {
            return Result.ok().put("data", PageResponse.<String>builder()
                    .counts(0L).current(0).size(0).total(0L).build());
        }
        ...
        return Result.ok().put("data", PageResponse.<String>builder()
                .counts(Long.valueOf(transportTaskDTO.getTransportOrderCount())).current(1).size(transportTaskDTO.getTransportOrderCount()).total(1L)
                .items(result).build());
    }
