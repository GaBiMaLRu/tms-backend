/*
 ...
 Source Schema         : tms_work

 Target Server Type    : MySQL
 Target Server Version : 80029
 File Encoding         : 65001

 Date: 16/07/2023 23:16:31
*/
...
CREATE TABLE `driver_job`  (
  ...
  `status` tinyint(0) NOT NULL COMMENT '作业状态，1为待执行（对应 待提货）、2为进行中（对应在途）、3为改派（对应 已交付）、4为已完成（对应 已交付）、5为已作废',
...
-- ----------------------------
-- Table structure for task_pickup_dispatch
-- ----------------------------
DROP TABLE IF EXISTS `task_pickup_dispatch`;
CREATE TABLE `task_pickup_dispatch`  (
  `id` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT 'id',
  `order_id` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '关联订单id',
  `task_type` tinyint(0) NULL DEFAULT NULL COMMENT '任务类型，1为取件任务，2为派件任务',
  `status` int(0) NULL DEFAULT NULL COMMENT '任务状态，1为待执行（对应 待上门和须交接）、2为进行中（该状态暂不使用，属于保留状态）、3为待确认（对应 待妥投和须交件）、4为已完成、5为已取消',
  `sign_status` tinyint(0) NULL DEFAULT NULL COMMENT '签收状态(1为已签收，2为拒收)',
  ...
  `assigned_status` tinyint(0) NOT NULL COMMENT '任务分配状态(1未分配2已分配3待人工分配)',
...
CREATE TABLE `task_transport`  (
  ...
  `start_agency_id` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '起始机构id',
  `end_agency_id` varchar(20) CHARACTER SET utf8mb4 COLLATE utf8mb4_general_ci NOT NULL COMMENT '目的机构id',
  `status` int(0) NOT NULL COMMENT '任务状态，1为待执行（对应 未发车）、2为进行中（对应在途）、3为待确认（保留状态）、4为已完成（对应 已交付）、5为已取消',
  `assigned_status` tinyint(0) NOT NULL COMMENT '任务分配状态(1未分配2已分配3待人工分配)',
  `loading_status` int(0) NOT NULL COMMENT '满载状态(1.半载2.满载3.空载)',
...
CREATE TABLE `transport_order`  (
  ...
  `status` int(0) NULL DEFAULT NULL COMMENT '运单状态(1.新建 2.已装车 3.到达 4.到达终端网点 5.已签收 6.拒收)',
  `scheduling_status` int(0) NULL DEFAULT NULL COMMENT '调度状态(1.待调度2.未匹配线路3.已调度)',
  `create_time` datetime(0) NOT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE = InnoDB CHARACTER SET = utf8mb4 COLLATE = utf8mb4_general_ci COMMENT = '运单表' ROW_FORMAT = Compact;

-- ----------------------------
-- Table structure for transport_order_task
-- ----------------------------
DROP TABLE IF EXISTS `transport_order_task`;
CREATE TABLE `transport_order_task`  (
